...
impl ConnectionManager {
        ...
        let handle = tokio::spawn(async move {
            ...
            loop {
                ...
                // 检查连接状态
                {
                    ...
                    for (id, connection) in connections_guard.iter_mut() {
                        ...
                        if connection.state == ConnectionState::Busy {
                            if now.duration_since(connection.last_used).as_secs() > 30 {
                ...
                // 更新统计
                {
                    ...
                    stats_guard.active_connections = connections_guard.len() - stats_guard.idle_connections;
                }

                debug!("Connection cleanup completed: removed {}, moved to idle: {}", 
                       to_remove.len(), to_idle.len());
            }
        });
    ...
    /// 获取连接
    pub async fn get_connection(&self, user_id: Option<String>, database: Option<String>) -> Result<Uuid> {
    ...
    /// 创建新连接
    async fn create_connection(&self, user_id: Option<String>, database: Option<String>) -> Result<Uuid> {
    ...
    /// 等待可用连接
    async fn wait_for_connection(&self, user_id: Option<String>, database: Option<String>) -> Result<Uuid> {
        ...
        loop {
            ...
            // 尝试直接创建或获取连接
            if self.can_create_new_connection().await {
                let connection_id = self.create_connection(user_id.clone(), database.clone()).await?;
