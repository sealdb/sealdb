...
use crate::executor::execution_models::{QueryResult, ExecutionContext};
use crate::storage::handler::StorageHandler;
use crate::storage::common::EngineType;
...
impl StorageExecutor {
    ...
    /// 注册存储引擎
    pub async fn register_storage_engine(&self, engine_type: EngineType, config: crate::storage::common::StorageConfig) -> Result<()> {
    ...
    pub async fn execute_table_scan(
        ...
        let engine_type = context.get_storage_engine_type().unwrap_or(self.default_engine_type);
        let result = self.storage_handler.scan_table(table_name, columns, limit, Some(engine_type)).await?;

        debug!("表扫描完成: {} 行", result.row_count);
        Ok(result)
    }
    ...
    pub async fn execute_point_query(
        ...
        debug!("点查询完成: {} 行", result.row_count);
        Ok(result)
    }
                ...
                StorageOperationType::Select => {
                    ...
                    match self.storage_handler.point_query(&table_name, &key, Some(engine_type)).await {
                        Ok(query_result) => StorageOperationResult {
                            operation_id: operation.operation_id,
                            success: true,
                            affected_rows: query_result.row_count as u64,
                            error: None,
                        },
                        Err(e) => StorageOperationResult {
                            operation_id: operation.operation_id,
