...
use crate::common::*;
use crate::engine::{StorageEngine, StorageEngineFactory};
...
impl StorageClient {
        ...
        Fut: std::future::Future<Output = Result<T>> + Send,
    {
        ...
        Err(last_error.unwrap_or_else(|| {
            StorageError::Internal("Max retries exceeded".to_string()).into()
        }))
    }
    ...
    pub async fn get(
        ...
        self.execute_with_retry(|| async {
            ...
            engine.get(key, &context, &options).await
        }).await
    }
    ...
    pub async fn execute_plan(
        ...
        context: Option<StorageContext>,
        options: Option<StorageOptions>,
    ) -> Result<StorageResult<StoragePlanResult>> {
        let context = context.unwrap_or_else(|| self.default_context.clone());
        let options = options.unwrap_or_else(|| self.default_options.clone());

        self.execute_with_retry(|| async {
            let engine = self.get_engine(engine_type).await?;
            engine.execute_plan(plan, &context, &options).await
        }).await
    }
