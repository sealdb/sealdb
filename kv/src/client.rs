...
use tikv_client::{RawClient, Key, Value as TiKVValue};
...
impl TiKVClient {
    ...
    pub async fn scan(&self, start_key: &[u8], end_key: &[u8], limit: u32) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
        ...
        match self.client.scan(start_key..end_key, limit).await {
            Ok(pairs) => {
                let result: Vec<(Vec<u8>, Vec<u8>)> = pairs
                    .into_iter()
                    .map(|pair| (pair.key().clone().into(), pair.value().clone().into()))
                    .collect();
