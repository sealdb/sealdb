...
impl TiKVTransaction {
    ...
    pub async fn get(&mut self, _key: &[u8]) -> Result<Option<Vec<u8>>> {
        Err(Error::Transaction("Transaction not implemented yet".to_string()))
    }
    ...
    pub async fn scan(&mut self, _start_key: &[u8], _end_key: &[u8], _limit: u32) -> Result<Vec<(Vec<u8>, Vec<u8>)>> {
        Err(Error::Transaction("Transaction not implemented yet".to_string()))
    }
