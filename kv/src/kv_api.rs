...
pub trait KVStore: Send + Sync {
    ...
    /// 扫描键值对
    async fn scan(&self, start_key: &[u8], end_key: &[u8], limit: u32) -> Result<Vec<(Vec<u8>, Vec<u8>)>>;
...
pub trait KVTransaction: Send + Sync {
    ...
    /// 事务内扫描键值对
    async fn scan(&mut self, start_key: &[u8], end_key: &[u8], limit: u32) -> Result<Vec<(Vec<u8>, Vec<u8>)>>;
